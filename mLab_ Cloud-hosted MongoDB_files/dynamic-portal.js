  ...
  var processResponse =
    function(data) {
      if($("#servers").length) {
        ...
        // populate replica set status
        if(data["rsStatus"]) {
          $("#replicaSetStatus").html(JSON.stringify(data["rsStatus"], null, "  "));
        }
        // populate servers
        var members = data["members"];
        if(members) {
          var numNonNascentNodes = 0;
          // clear footnotes first
          $("#serverFootnotes").empty();
          $("#servers .serverRow .hiddenFootnoteMarker").empty();
          for(var i=0; i < members.length; i++) {

            var member = members[i];
            var $row = $("#"+member["id"]);

            if(!(member && member["server"] && member["server"]["tags"] && member["server"]["tags"]["clonedFrom"])) {
              numNonNascentNodes++;
            }
            if($row.length && member) {
              if(member["underMaintenance"]) {
                $row.addClass("replStateMaintenance");
                $row.find(".memberOptime").html("");
                $row.find(".memberUptime").html("");
                $row.find(".memberStatus").html("under maintenance");
              } else if(member["selfReplStatus"]) {
                ...
                if($row.attr("hiddenNode") === "true") {
                  $row.find(".hiddenFootnoteMarker").html("<sup>"+addFootnote("Server is hidden and not eligible to be primary")+"</sup>");
                } else if($row.attr("priority") === "0") {
                  $row.find(".hiddenFootnoteMarker").html("<sup>"+addFootnote("Server is not eligible to be primary")+"</sup>");
                }
